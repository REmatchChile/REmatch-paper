...
def run_bench(binary_data, document_path, regex_path, n_runs, versions_binary=False):
    ...
    memory = int(re.search(r"(\d+)\s+maximum resident set size", stderr).group(1))
    if versions_binary:
        n_outputs = int(
...
def run_experiments(benchmark: str):
    ...
    for suite in CHOSEN_EXPERIMENTS:
        ...
        for dataset in sorted(next(os.walk(sample_dir, followlinks=True))[1]):
            for experiment in sorted(
                ...
                astats = automata_stats(
                    document_path, os.path.join(experiment, "rematch.rgx")
                )
