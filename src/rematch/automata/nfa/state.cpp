...
LogicalVA::Capture::Capture(State* from, std::bitset<32> coding, State* next)
    : from(from), next(next), code(coding), flag(false) {}
...
LogicalVA::State::State() : flags_(kDefaultState) {
  id = ID++;
}

LogicalVA::State::State(const State& s) : flags_(s.flags_) {
  id = ID++;
}
...
bool LogicalVA::State::operator==(const State& rhs) const {
  return id == rhs.id;
}
