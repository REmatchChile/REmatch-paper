...
ExtendedVA::ExtendedVA(LogicalVA const& A, Anchor a)
    : variable_factory_(A.varFactory()), filter_factory_(A.filterFactory()), anchor_(a) {
  // Copy the VA
  LogicalVA A_prim(A);
...
bool ExtendedVA::is_ambiguous() const {
  ...
  struct StatePair {
    ...
    bool operator==(const StatePair& rhs) const {
      return (p1->id == rhs.p1->id) && (p2->id == rhs.p2->id) && (splitted == rhs.splitted);
    }
  };

  struct StatePairHasher {
    size_t operator()(const StatePair& sp) const {
      std::string s = std::to_string(sp.p1->id) + " " + std::to_string(sp.p2->id) + " " + std::to_string(sp.splitted);
    ...
    for (auto& f1 : spair.p1->filters) {
      for (auto& f2 : spair.p2->filters) {
        if (!intersects[{std::min(f1->code, f2->code), std::max(f1->code, f2->code)}].empty()) {
          ...
          auto rt = visited.insert(new_pair);
          if (rt.second) {  // if insert was successful
            queue.push(new_pair);
          }
        }
      }
...
void ExtendedVA::cleanUselessCaptureStates() {
  ...
  for (auto state = states.begin(); state != states.end();) {
    ...
    isUselessCaptureState = ((*state)->backward_filters_.empty() && (*state)->filters.empty() &&
  ...
  while (offset_possible(captureList)) {
    for (auto it = captureList.begin(); it != captureList.end();) {
      ...
      for (auto& filter : q->filters) {
        ...
        // Create new state q'
        auto* q_prim = new LogicalVA::State();
...
bool ExtendedVA::offset_possible(CapturePtr capture) {
  ...
  if (q->accepting())
    return false;
  if (q->filters.empty() || !q->captures.empty())
    return false;
...
bool ExtendedVA::isReachable(LogicalVA::State* from, LogicalVA::State* to) {
  for (auto& state : states)
...
std::queue<LogicalVA::State*> ExtendedVA ::invTopologicalSort() {
  auto* Q = new std::queue<LogicalVA::State*>();
