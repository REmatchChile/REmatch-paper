...
Span Match::span(const std::string& var) const {
  try {
...
std::string_view Match::group(const std::string& var) const {
  try {
    int pos = var_factory_->position(var);
    return document_.substr(data_[pos * 2],
                            data_[pos * 2 + 1] - data_[pos * 2]);
  } catch (...) {
    throw std::logic_error("No mapping assigned to variable.");
  }
}
...
int64_t Match::start(const std::string& var) const {
  try {
    int pos = var_factory_->position(var);
    return data_[pos * 2];
  } catch (...) {
...
int64_t Match::end(const std::string& var) const {
  try {
    int pos = var_factory_->position(var);
