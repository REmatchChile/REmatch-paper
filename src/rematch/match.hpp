...
// It doesn't store the corresponding substrings, so it's assumed that the
...
class Match {
 ...
 public:
  ...
  int64_t start(const std::string& varname) const;
  int64_t end(const std::string& varname) const;

  Span span(const std::string& var) const;

  // Returns a variable's captured substring
  std::string_view group(const std::string& var) const;
