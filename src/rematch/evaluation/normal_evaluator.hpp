...
class NormalEvaluator : public Evaluator {
 ...
 public:
  NormalEvaluator(RegEx& rgx, std::string_view d, Anchor a, EvalStats& e);
 ...
 private:
  inline void reading(char a, int64_t pos);

  inline void visit(abstract::DState* ns, ECS::Node* passed_node, int64_t pos,
  ...
  void init_evaluation_phase(int64_t pos);
...
template <typename A>
NormalEvaluator<A>::NormalEvaluator(RegEx& rgx, std::string_view d, Anchor a,
                                    EvalStats& e)
    : rgx_(rgx),
...
inline void NormalEvaluator<A>::reading(char a, int64_t pos) {
  ...
  for (auto& p : current_states_) {
    auto dp = static_cast<DFA::State*>(p);
#ifdef NOPT_ASCIIARRAY
    auto nextTransition = dfa_->next_base_transition(dp, a);
#else
    auto nextTransition = dfa_->next_transition(dp, a);
#endif
    ...
    switch (nextTransition.type_) {
      case A::Transition::Type::kDirect:
        visit(d, from_node, pos);
        break;
      case A::Transition::Type::kDirectSingleCapture:
        visit(d, from_node, pos, false);
        from_node->inc_ref_count();
        visit(c.next, ds_.extend(from_node, c.S, pos + 1), pos);
        break;
      case A::Transition::Type::kEmpty:
        from_node->dec_ref_count();
        ds_.try_mark_unused(from_node);
        break;
      case A::Transition::Type::kSingleCapture:
        visit(c.next, ds_.extend(from_node, c.S, pos + 1), pos);
        break;
      case A::Transition::Type::kMultiDirect:
        visit(d, from_node, pos, false);
        ...
        break;
      case A::Transition::Type::kMultiDirectSingleCapture:
        visit(d, from_node, pos, false);
        for (auto& q : nextTransition.directs_) {
          from_node->inc_ref_count();
          visit(q, from_node, pos, false);
        ...
        visit(c.next, ds_.extend(from_node, c.S, pos + 1), pos);
        break;
      case A::Transition::Type::kDirectMultiCapture:
        visit(d, from_node, pos, false);
        ...
        break;
      case A::Transition::Type::kMultiDirectMultiCapture:
        visit(d, from_node, pos, false);
        for (auto& q : nextTransition.directs_) {
          from_node->inc_ref_count();
          visit(q, from_node, pos, false);
...
inline void NormalEvaluator<A>::visit(abstract::DState* ns,
                                      ECS::Node* passed_node, int64_t pos,
                                      bool garbage_left) {
  if (ns->visited() <= pos) {
