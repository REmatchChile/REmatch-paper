...
SegmentEvaluator::SegmentEvaluator(RegEx& rgx, std::string_view d, Anchor a,
                                   EvalStats& e)
    : rgx_(rgx), enumerator_(rgx_, d), text_(d), anchor_(a), stats_(e) {
...
#ifndef NOPT_SEGMENT_RATIO
  if (i_src_ < text_.size() && !good_segment_ratio()) {
    i_max_ = text_.size();
    i_src_ = text_.size();
    return true;
  }
  ...
  for (; i_src_ < text_.size(); ++i_src_) {
    ...
    if (current_dstate_->accepting())
      ...
      if (i_min_ < i_max_) {
        ...
        segments_tot_size_ += i_max_ - i_min_;
        return true;
      }
      i_min_ = i_src_;
    }
...
inline void SegmentEvaluator::reading(char a, int64_t pos) {
  new_states_.clear();
  ...
  for (auto& p : current_states_) {
    ...
    switch (nextTransition.type_) {
      case DFA::Transition::Type::kDirect:
        visit(d, from_node, pos);
        break;
      case DFA::Transition::Type::kDirectSingleCapture:
        visit(d, from_node, pos, false);
        from_node->inc_ref_count();
        visit(c.next, ds_.extend(from_node, c.S, pos + 1), pos);
        break;
      case DFA::Transition::Type::kEmpty:
        from_node->dec_ref_count();
        ds_.try_mark_unused(from_node);
        break;
      case DFA::Transition::Type::kSingleCapture:
        visit(c.next, ds_.extend(from_node, c.S, pos + 1), pos);
        break;
      case DFA::Transition::Type::kMultiDirect:
        visit(d, from_node, pos, false);
        ...
        break;
      case DFA::Transition::Type::kMultiDirectSingleCapture:
        visit(d, from_node, pos, false);
        for (auto& q : nextTransition.directs_) {
          from_node->inc_ref_count();
          visit(q, from_node, pos, false);
        ...
        visit(c.next, ds_.extend(from_node, c.S, pos + 1), pos);
        break;
      case DFA::Transition::Type::kDirectMultiCapture:
        visit(d, from_node, pos, false);
        ...
        break;
      case DFA::Transition::Type::kMultiDirectMultiCapture:
        visit(d, from_node, pos, false);
        for (auto& q : nextTransition.directs_) {
          from_node->inc_ref_count();
          visit(q, from_node, pos, false);
...
bool SegmentEvaluator::good_segment_ratio() const {
  if (i_src_ == 0)
    return true;

  double segment_ratio = i_src_ - i_min_ + segments_tot_size_;

  return segment_ratio <= MAX_SEGMENT_RATIO * i_src_ ||
         i_src_ <= MIN_PREFIX_READ;
}
